...
<body>
    <div class="container-fluid position-relative top-0 w-100 bg-warning bg-gradient text-dark shadow rounded-bottom-5">
        <div class="row" style="height: 15vh !important">
            <div class="col-4 d-flex align-items-center justify-content-start">
                <img src="{{ URL::asset('/admin/dist/img/logo.png') }}" alt="AU Robo Academy Logo" style="height: 12vh;">
            </div>
            ...
            <div class="col-4 d-flex align-items-center justify-content-end">
                <a href="#" onclick="{{ request()->segment(1) == 'student' ? 'closeWindow()' : 'goBack()' }}">
                    <i
                        class="fas fa-3x {{ request()->segment(1) == 'student' ? 'fa-close' : 'fa-reply' }} text-danger"></i>
                </a>
            </div>
        </div>
    </div>
    ...
    <div class="container-fluid ps-5">
        <div class="row gx-5 gx-5 pt-2 d-flex align-items-center justify-content-center w-100" style="height: 70vh !important">
            <div class="col-md-10 position-relative">
                <div class="bg-dark rounded-5 shadow" style="height: 55vh;">
                    <div class="slider-for">
                        @foreach ($modelSteps as $modelStep)
                            <div class="row imageDev ">
                                <div class="col-12 col-md-6 d-flex offset-md-3 modelViewer" id="modelViewer">
                                    ...
                                    <img class="w-100 imgSrc img-fluid" id="Image"
                                        src="{{ isset($modelStep->image) ? url($modelStep->image) : '' }}"
                                        data-modelstep_id="{{ $modelStep->id }}"
                                        data-modeltitle="{{ $modelStep->title }}"
                                        style="height: 55vh;">
                                    <p class="text-light 3d_not_found d-flex justify-content-center d-none"
                                        style="margin-top: 25%;">3D Image Not Available</p>
                                </div>
                            </div>
                        @endforeach
                    </div>
            ...
            <div class="col-md-2">
                <div class="bg-success-subtle shadow p-2 d-grid align-items-center justify-content-center rounded-5"
                    style="height: 50vh">
                    <div class="totalCount">
                        <span id="startSteps"
                            class="bg-secondary fw-bold p-2 ps-5 pe-5 text-light rounded-pill">1</span>
                    </div>
                    <i class="fas fa-4x fa-info-circle model-info text-center" id="stepInformation"
                        class="btn rounded-0" data-bs-toggle="modal" data-bs-target="#info"></i>
    ...
    <div class="container-fluid position-fixed bottom-0 w-100 bg-info shadow text-light" id="slideBar">
        <div class="row" style="height: 15vh !important">
            <div class="col-12">
                <div class="slider-nav ">
                    @foreach ($modelSteps as $modelStep)
                        <img class="img-fluid rounded-4 m-2 slideImages" data-slider-show="show"
                            src="{{ url($modelStep->image) }}" style="height: 13vh;">
                    @endforeach
                </div>
            </div>
        </div>
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content bg-success-subtle bg-gradient    shadow-lg">
                <div class="modal-header">
                    <h1 class="modal-title fs-5" id="staticBackdropLabel">Model Step ID: <span id="modelStepId"></span>
                    </h1>
