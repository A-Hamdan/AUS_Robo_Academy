...
@section('title', $title)
@section('content')
    {{-- <x-content-header title='{{ $title }}' icon='video' link='true'>{{ route('video.create') }}</x-content-header> --}}
    <x-content-header title='{{ $title }}' icon='video' link='true'>{{ auth()->user()->hasRole(['super-admin','admin']) ? route('video.create') : 'javascript:history.back()' }}</x-content-header>
